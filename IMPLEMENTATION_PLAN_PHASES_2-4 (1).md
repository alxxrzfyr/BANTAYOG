...
### 2. Google Cloud — Vision API (Product Search) (Required before Phase 3)

Google Cloud Vision Product Search is used to identify products from photos taken by merchants. It does **not** determine eligibility — that is determined by cross-referencing the identification result against the Supabase `products` table.

**What to create / configure:**

1. **Create a Google Cloud project:**
   - Go to [https://console.cloud.google.com](https://console.cloud.google.com) and sign in.
   - Click **"Select a project" → "New Project"**. Name it (e.g., `bantayog-vision`). Click **Create**.

2. **Enable the Vision API:**
   - In the Google Cloud Console, go to **APIs & Services → Library**.
   - Search for **"Cloud Vision API"** and click **Enable**.

3. **Set up Product Search resources:**
   - Go to **Vision API → Product Search** in the console (or use the `gcloud` CLI).
   - Create a **Product Set** (this is the searchable catalog of reference images):
     ```bash
     gcloud ml vision product-search product-sets create \
       --product-set-id=bantayog-products \
       --display-name="Bantayog Product Catalog" \
       --location=asia-southeast1
     ```
   - For each product in your catalog, create a **Product** and upload **Reference Images**:
     ```bash
     # Create a product
     gcloud ml vision product-search products create \
       --product-id=product-001 \
       --display-name="Alaska Evaporada 370ml" \
     ...
     gcloud ml vision product-search reference-images create \
       --product-id=product-001 \
       --location=asia-southeast1 \
       --uri=gs://bantayog-vision-images/alaska-evaporada-370ml-01.jpg
     ```
   - **Index the Product Set** (this happens automatically but can take up to 30 minutes after adding images).

4. **Create a Service Account:**
   - Go to **IAM & Admin → Service Accounts**.
   - Click **"Create Service Account"**. Name it `bantayog-vision-sa`.
   - Grant the role **"Cloud Vision API User"** (`roles/vision.user`).
   - Click **"Create Key" → JSON**. Download the JSON key file.
   - Save the key file securely (e.g., as `google-vision-sa-key.json` in your server's secrets directory — **never commit this file**).

5. **Create a Cloud Storage bucket for reference images:**
   - Go to **Cloud Storage → Create Bucket**.
   - Name it (e.g., `bantayog-vision-images`), select the same region as your Product Search resources.
   - Upload reference product images to this bucket.

**Environment variables to set:**

```env
GOOGLE_APPLICATION_CREDENTIALS=/path/to/google-vision-sa-key.json
GOOGLE_CLOUD_PROJECT=bantayog-vision
GOOGLE_CLOUD_LOCATION=asia-southeast1
GOOGLE_PRODUCT_SET_ID=bantayog-products
```

---

### 3. Ronin Blockchain — Saigon Testnet / Mainnet (Required before Phase 2)

The Ronin blockchain is used for the PHPC stablecoin, beneficiary registry, and merchant registry smart contracts.

**What to create / configure:**

1. **Install a Ronin-compatible wallet:**
   - Install the **Ronin Wallet** browser extension from [https://wallet.roninchain.com](https://wallet.roninchain.com).
   - Create a new wallet or import an existing one. **Securely back up your seed phrase.**

2. **Configure for Saigon Testnet (development):**
   - In Ronin Wallet, switch to the **Saigon Testnet** network.
   - Get testnet RON from the [Ronin Faucet](https://faucet.roninchain.com) for gas fees.
   - Note your deployer wallet address and private key.

3. **Create required wallets/accounts:**
   - **Deployer wallet:** The wallet that deploys smart contracts. Needs RON for gas.
   - **LGU Treasury wallet:** The wallet that holds the LGU's PHPC allocation. This is the wallet checked for balance when the admin clicks "Add Credits".
   - **Merchant wallets:** Each merchant gets a wallet address during registration. For v1, this can be a custodial address generated server-side or provided by the merchant via Waypoint/Tanto.
...
4. **Deploy smart contracts:**
   - From the `packages/contracts/` directory:
     ...
     ```
   - Record the deployed contract addresses for `PHPC`, `PHPCSubsidy`, `BeneficiaryRegistry`, `MerchantRegistry`.
...
flowchart TD
    ...
    CAPTURE --> UPLOAD["2. Photo uploaded to Supabase Storage (cart-photos bucket)"]
    UPLOAD --> API["3. POST /api/vision/classify — photo forwarded to backend"]
    API --> VISION["4. Backend calls Google Cloud Vision Product Search"]

    VISION --> LEGIBLE{"5. Is the image legible and product recognizable?"}

    LEGIBLE -->|"Yes — Vision returns product match(es)"| MATCH["6. Backend receives Vision product ID + confidence score"]
    LEGIBLE -->|"No — Image illegible / unrecognizable"| REJECT_ILLEGIBLE["❌ REJECTED: Image illegible or unrecognizable — item NOT added to cart"]

    MATCH --> DB_QUERY["7. Backend queries Supabase products table WHERE name/id matches Vision result"]

    DB_QUERY --> DB_FOUND{"8. Match found in Supabase products table?"}

    DB_FOUND -->|"Yes — Product exists in catalog"| CHECK_ELIG["9. Read eligibility_status from matched product row"]
    DB_FOUND -->|"No — No matching product in catalog"| REJECT_NO_MATCH["❌ REJECTED: Product not in database — defaults to Ineligible — item NOT added to cart"]

    CHECK_ELIG --> ELIG_STATUS{"10. eligibility_status = ?"}

    ELIG_STATUS -->|"eligible"| ACCEPT["✅ ACCEPTED: Item displayed with 'Eligible' tag — can be added to cart"]
    ELIG_STATUS -->|"ineligible"| REJECT_INELIG["❌ REJECTED: Item displayed with 'Ineligible' tag — auto-rejected from cart"]

    ACCEPT --> REVIEW["11. AI Review screen: merchant can edit name/price/qty"]
    REVIEW --> ADD_CART["12. 'Add to Cart' — only Eligible items carried forward"]

    REJECT_ILLEGIBLE --> REVIEW_FAIL["11a. AI Review screen shows rejection reason — merchant can retry or switch to Manual Input"]
    ...
    class LEGIBLE,DB_FOUND,ELIG_STATUS decision
```
...
2. **Photo uploaded:** The captured image is uploaded to Supabase Storage in the `cart-photos` bucket. A signed URL or the raw file is forwarded to the backend.
3. **Backend receives request:** The frontend calls `POST /api/vision/classify` with the image reference.
4. **Google Cloud Vision Product Search:** The backend sends the image to Google Cloud Vision Product Search API, which compares it against the pre-indexed Product Set (`bantayog-products`). Vision's job is **identification only** — it returns what the product likely is, with a confidence score.
5. **Legibility check:** If the image is too blurry, dark, or otherwise illegible, Vision will return no results or very low confidence scores (below threshold). The backend treats this as **unrecognizable** and returns a rejection.
6. **Vision returns match(es):** If Vision successfully identifies the product, it returns the product ID and confidence score from the Product Set.
7. **Supabase product lookup:** The backend queries the Supabase `products` table to find a matching record. This lookup uses the product name/ID returned by Vision to find the corresponding row in the catalog.
8. **Database match check:** If no matching product exists in the Supabase `products` table, the item defaults to **Ineligible** — it is not a recognized product in the program's catalog and cannot be subsidized.
9. **Read eligibility status:** If a match is found, the backend reads the `eligibility_status` column from the matched product row. This is the **authoritative** source of eligibility — not Vision, not the client.
10. **Eligibility determination:**
    - `eligible` → The item is a valid subsidizable product. It is returned to the frontend with an "Eligible" tag.
    - `ineligible` → The item is recognized but not eligible for subsidy. It is returned with an "Ineligible" tag.
11. **AI Review screen:** All results (accepted and rejected) are displayed on the AI Review screen. The merchant can see why items were rejected. Eligible items can have their name, price, and quantity edited. Ineligible items are shown but cannot be added to the cart.
12. **Add to Cart:** Only items tagged as "Eligible" are carried into the Cart Summary. Ineligible items are auto-rejected by the cart-filtering logic.
...
>
> 1. **Product identification** (Vision API for AI scan, merchant text input for manual)
...
| **Acceptance** | `npx hardhat run scripts/deploy.ts --network saigon` deploys all contracts successfully. UUPS upgrade test passes. |
...
#### BE2-3.1 · Vision Service (Google Cloud Vision Product Search + Supabase Match)

| | |
|---|---|
| **Assigned to** | 🤖 AI |
| **File** | `packages/server/src/services/vision.service.ts` |
| **Description** | Implements the AI product identification pipeline. `classifyProduct(imageUrl: string)`: (1) Downloads the image from Supabase Storage, (2) Calls Google Cloud Vision Product Search API with the image, specifying the `bantayog-products` Product Set, (3) Receives identified product(s) with confidence scores, (4) Filters results below a configurable confidence threshold (e.g., 0.7), (5) For each identified product, queries the Supabase `products` table for a matching record, (6) Reads the `eligibility_status` from the matched row, (7) Returns the classification result: `{ identified: true, product: { name, confidence, eligibility_status } }` or `{ identified: false, reason: 'unrecognizable' }`. Uses `p-retry` for resilient API calls with exponential backoff. The Vision API does **identification only** — eligibility is always determined by the Supabase product catalog. |
| **Depends on** | Google Cloud Vision setup, Supabase `products` table |
| **Acceptance** | Known products are identified with correct eligibility from Supabase. Unknown products return `identified: false`. Low-confidence results are filtered. Retries work on transient failures. |
...
| **Description** | `POST /api/vision/classify` — accepts `{ imageUrl: string }` (the Supabase Storage URL of the uploaded photo). Calls `vision.service.classifyProduct()`. Returns the classification result with product name, confidence, and eligibility status. Handles errors from the Vision API (rate limits, quota exceeded, invalid image) with appropriate HTTP status codes and user-friendly error messages. |
| **Depends on** | BE2-3.1 |
| **Acceptance** | Route accepts image URLs and returns classification results. Error handling covers all Vision API failure modes. |
...
| **Description** | Two-step AI scanning flow. **Step 1 — Capture:** Opens the device camera for the merchant to photograph a product. On capture, the photo is uploaded to Supabase Storage (`cart-photos` bucket). The Supabase Storage URL is sent to `POST /api/vision/classify`. **Back button (`<`)** on the capture screen returns to the Scan Cart Items method choice page. **Step 2 — AI Review:** Displays the classification results from the backend. For each detected item, shows: product name, price, quantity, and an **Eligible / Ineligible** tag (sourced from the Supabase product catalog match, NOT from the Vision API itself). All fields (name, price, qty) are editable by the merchant. Tags are also shown but reflect the backend determination. "Add to Cart" button adds only Eligible items to the Zustand cart store. Ineligible items are visually distinguished (e.g., greyed out, strikethrough) and cannot be added. If the image was illegible or no products were recognized, show an appropriate message with option to retry or switch to Manual Input. **Back button (`<`)** on the review screen returns to the Scan Cart Items method choice page. |
| **Depends on** | BE2-3.1, BE2-3.2, Supabase Storage |
| **Acceptance** | Photo is taken and uploaded. Vision API identifies the item. Backend cross-references the Supabase product catalog. Items displayed with correct Eligible/Ineligible tag from Supabase. Fields are editable. Only Eligible items can be added to cart. Illegible images show error with retry option. Both back buttons navigate correctly. |
...
| **Assigned to** | 🤖 AI |
| **Scope** | Monorepo-wide |
| **Description** | Install all Phase 3 dependencies: `xstate` (v5, state machine), `@google-cloud/vision` (Cloud Vision API), `p-retry` (resilient API calls), `@sky-mavis/waypoint` + `@sky-mavis/tanto-connect` (wallet adapters). Configure XState and Vision API client setup. |
...
| Person | Tasks | Focus Area |
|---|---|---|
| 🤖 **AI** | BE1-3.1 through BE1-3.5, BE2-3.1 through BE2-3.7, Dependency Installation | Transaction pipeline, state machine, Vision API integration, product validation, chain client, wallet gateway, DTOs, snapshot tests, e2e integration test |
...
flowchart LR
    subgraph BE["Backend Deliverables"]
        ...
        BE_VIS["BE2: Cloud Vision Product Search + Supabase Eligibility Match"]
...
| AI vision | `@google-cloud/vision` (Product Search) | latest | **P3** | To be installed; identification only, eligibility from Supabase |
