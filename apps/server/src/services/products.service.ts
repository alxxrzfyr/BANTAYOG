...
export class ProductsService {
   ...
   */
  async validateProduct(name: string): Promise<ValidationResult> {
    if (!name || name.trim() === '') {
      return { matched: false, reason: 'Empty product name provided' }
    }
    ...
    try {
      ...
      if (!results || results.length === 0) {
        return { matched: false, reason: 'Product not found in catalog' }
      }
      ...
      return { matched: false, reason: `Database lookup failed: ${error.message}` }
    }
  }
}
